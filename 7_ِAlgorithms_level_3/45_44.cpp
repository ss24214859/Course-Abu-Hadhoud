...
string ReplaceWordInString(string S1, string OldStr, string StrReplaceTo, bool IsMatchCase = true)
{
    string deli = " ";
    vector<string> vSWords = SplitStringInVector(S1, deli);
    short pos = 0;
    string LowS1 = LowerAllString(S1);
    ;

    pos = (IsMatchCase) ? S1.find(OldStr) : LowS1.find(LowerAllString(OldStr));
...
int main()
{
    ...
    cout << "\n\nRemove Punctuations : " << RemoveAllPunctuationsInString(S1) << endl;
