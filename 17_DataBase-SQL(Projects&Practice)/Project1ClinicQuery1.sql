...
CREATE TABLE "Prescription"(
    ...
    "dosage" NVARCHAR(200) NOT NULL,SELECT 
FROM     Appointment INNER JOIN
                  MedicalRecord ON Appointment.MedicalRecordID = MedicalRecord.MedicalRecordID INNER JOIN
                  Doctor ON Appointment.DoctorID = Doctor.DoctorID INNER JOIN
                  Patient ON Appointment.PatientID = Patient.PatientID INNER JOIN
                  Payment ON Appointment.PaymentID = Payment.PaymentID INNER JOIN
                  Person ON Doctor.PersonID = Person.PersonID AND Patient.PersonID = Person.PersonID INNER JOIN
                  Prescription ON MedicalRecord.MedicalRecordID = Prescription.MedicalRecordID
